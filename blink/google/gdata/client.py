...
from blink.google.atom import client as atom_client, core as atom_core, http_core as atom_http_core
from blink.google.gdata import data as gdata_data, gauth as gdata_gauth
...
def error_from_response(message, http_response, error_class,
  ...
  error.body = body
  error.headers = atom_http_core.get_headers(http_response)
  return error
...
class GDClient(atom_client.AtomPubClient):
  ...
  to use gdata_gauth.AuthSubToken.from_url to obtain the token after the
  ...
  def request_client_login_token(
    ...
    http_request.add_body_part(
        gdata_gauth.generate_client_login_request_body(email=email,
    ...
    if response.status == 200:
      token_string = gdata_gauth.get_client_login_token_string(response_body)
      ...
      captcha_challenge = gdata_gauth.get_captcha_challenge(response_body)
...
class GDQuery(atom_http_core.Uri):
...
class ResumableUploader(object):
  ...
  def __init__(self, client, file_handle, content_type, total_file_size,
    ...
    Args:
      ...
      desired_class: object (optional) The type of gdata_data.GDEntry to parse
    ...
    self.desired_class = desired_class or gdata_data.GDEntry
  ...
  def update_file(self, entry_or_resumable_edit_link, headers=None, force=False,
    ...
    Args:
      entry_or_resumable_edit_link: object or string A gdata_data.GDEntry for
      ...
      force boolean (optional) True to force an update and set the If-Match
          ...
          gdata_data.GDEntry object, its etag value is used. Otherwise this
    ...
    if isinstance(entry_or_resumable_edit_link, gdata_data.GDEntry):
